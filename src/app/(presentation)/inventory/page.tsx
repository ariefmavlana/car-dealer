...
import { CLASSFIEDS_PER_PAGE } from "@/config/constants"
...
const buildClassifiedFilterQuery = (
  ...
  const enumFilters = ["odoUnit", "currency", "transmission", "bodyType", "fuelType", "colour", "ulezCompliant"]
  ...
  const mapParamsToFields = keys.reduce((acc, key) => {
    ...
    if (taxonomyFilters.includes(key)) {
      ...
      if (key.startsWith("min")) {
        acc[field].get = Number(value)
      } else if (key.startsWith('max')) {
        acc[field].let = Number(value)
      }
    }
    return acc
  }, {} as {[key: string]: any},
...
const getInventory = async (searchParams: AwaitedPageProps['searchParams']) => {
  ...
  // calculate the offset
	const offset = (page - 1) * CLASSFIEDS_PER_PAGE

  return prisma.classified.findMany({
    where: buildClassifiedFilterQuery(searchParams),
    include: { images: {take: 1} },
    skip: offset,
    take: CLASSFIEDS_PER_PAGE,
  })
}
...
export default async function InventoryPage(props: PageProps) {
  ...
  const totalPages = Math.ceil(count / CLASSFIEDS_PER_PAGE)
      ...
      <div className="flex-1 p-4 bg-white">
        <div className="flex space-y-2 flex-col lg:flex-row items-center justify-center pb-4 -mt-1">
          <div className="flex justify-between items-center w-full">
            <h2 className="text-sm text-gray-600 md:text-base lg:text-xl font-semibold min-w-fit">
